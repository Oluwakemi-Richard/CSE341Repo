...
const createContact = async (req, res) => {
  ...
  if (response.acknowledged) {
    ...
    res.status(500).json(response.error || 'Sorry, an error occured while creating contact.');
  }
};
...
const updateSingle = async (req, res) => {
  ...
  if (response.modifiedCount > 0) {
    ...
    res.status(500).json(response.error || 'Sorry, an error occured while updating contact.');
  }
};
